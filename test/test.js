var expect = require('chai').expect,
    Restup = require('../index.js'),
    api    = new Restup({ path: './services/' });
    
describe('REST API Calls', function() {
  
  describe('via HTTP GET', function() {
  
    it('should handle a JSON response from service', function(done) {
      api.call('latest', 'FixerIO', { sourceCurrency: 'USD', targetCurrencies: 'GBP' }, function(result) {
        expect(result.base).to.equal('USD');
        done();
      }); 
    });

    it('should handle an XML response from service', function(done) {
      api.call('callsign', 'Callook', { callsign: 'W1AW' }, function(result) {
  ...
  describe('via HTTP POST', function() {
    it('should handle a JSON response from service', function(done) {
      api.call('new', 'MyJSON', { data: { 'testKey': 'testVal' } }, function(result) {
        //console.log(result);
        expect(result.uri || '').to.match(/^https:\/\/api\.myjson\.com\/bins\/[a-z0-9]+$/);
        done();
      }); 
    });

    it('should handle an XML response from service', function(done) {
      api.call('post', 'HTTPBin', { id: 'ABC123XYZ' }, function(result, b, c) {
        console.log('!!!');
        console.log(result);
        console.log(b);
        console.log(c);
